...
battleAttackAttackSpec = prop "1ターン攻撃攻撃" $
    \properties ->
        let hp = properties ^. T.maxHp
            ...
            skills =[T.Skill "skill" (T.Attack P.factorDenominator) T.TcAliveOpponentOne]
...
battleAttackDefenseSpec = prop "1ターン攻撃防御" $
    \properties ->
        let hp = properties ^. T.maxHp
            ...
            attackSkills = [T.Skill "skill" (T.Attack P.factorDenominator) T.TcAliveOpponentOne]
            defenseSkills = [T.Skill "skill" (T.Defense defenseFactor) T.TcSelf]
...
battleAttackHealSpec = prop "1ターン攻撃回復" $
    \properties (Positive useMp) ->
        let hp = properties ^. T.maxHp
            ...
            attackSkills = [T.Skill "skill" (T.Attack P.factorDenominator) T.TcAliveOpponentOne]
            healSkills = [T.Skill "skill" (T.Heal healFactor useMp) T.TcSelf]
            attackCards = [T.Card "testCard" properties attackSkills]
...
battleBufSpec = prop "攻撃力Buf使用" $
    \properties (Positive turn) (Positive useMp) (Positive factor) ->
        let
            ...
            mp = properties ^. T.maxMp
            tag = T.AttackTag
            buffSkills = [T.Skill "skill" (T.Buff tag factor turn useMp) T.TcSelf]
